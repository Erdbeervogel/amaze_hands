  ...
  describe '#apply' do
    ...
    context 'wait times' do
      ...
      its(:wait_time) do
        is_expected.to eq(
          '2015-4' => {
            'Doing: Capability' => { item_values: [1.0], sum: 1.0, count: 1, mean: 1.0, median: 1.0 }
          },
          '2015-5' => {
            'QA'  => { item_values: [2.0], sum: 2.0, count: 1, mean: 2.0, median: 2.0 },
            'BAT' => { item_values: [3.0], sum: 3.0, count: 1, mean: 3.0, median: 3.0 }
          }
        )
      end
    end
